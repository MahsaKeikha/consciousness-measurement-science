...
The formal validation record is regenerated by eight reader-visible runners:
...
The first three layers use declared fixed seeds where sampling is stochastic. V16-V35 are deterministic. V36 uses fixed seed `20260918`; V37-V40 are analytic. Canonical CSV/JSON outputs are committed under `results/`, and validation figures are committed under `docs/figures/`. Reader-facing prose should be checked against the machine-readable records rather than treated as the numerical source of truth.
