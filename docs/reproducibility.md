...
The formal validation record is regenerated by six reader-visible runners:
...
The first three layers use declared fixed seeds where sampling is stochastic. V16-V30 are deterministic. Canonical CSV/JSON outputs are committed under `results/`, and validation figures are committed under `docs/figures/`. Reader-facing prose should be checked against the machine-readable records rather than treated as the numerical source of truth.
